...
import platform
import random
from data import greetings_to_learn  # Adjust according to your data structure
...
def get_instructional_content(greeting):
    try:
        response = client.chat.completions.create(
            model="gpt-3.5-turbo",
            messages=[{
                "role": "user", 
                "content": f"Explain the French greeting '{greeting}' and provide some examples of how it's used, including their translations in English."
            }],
            temperature=0.7,
            max_tokens=200,
            top_p=1.0,
...
def get_quiz_question(english_description):
    ...
    return f"\nBot: What is the French greeting for '{english_description}'?"
...
def greetings_lesson_teaching():
    print("\nBot: Hi! Welcome to the French greetings learning section.\n")

    greetings_list = list(greetings_to_learn.items())
    batch_size = 5
    batch_index = 0  # Initialize batch index

    while batch_index * batch_size < len(greetings_list):  # Ensure loop runs as long as there are batches
        start_index = batch_index * batch_size
        batch = greetings_list[start_index:start_index + batch_size]

        for j, (french_greeting, english_description) in enumerate(batch):
            intro_line = "Let's begin with" if j == 0 else transition_phrase()
            print(f"\n{intro_line} '{french_greeting}'.\n")
            print(get_instructional_content(french_greeting))
            input("\nPress Enter once you're ready to move on...\n")

        print("\nBot: Alright! It's quiz time 🌟 You'll need to translate the following English phrases into French. Hit Enter when you're ready.")
        input()
        clear_screen()

        correct_answers = 0
        for _, (french_greeting, english_description) in enumerate(batch):
            question = get_quiz_question(english_description)
            print(question)
            user_response = input("Your answer: ").strip()
            feedback, correct = evaluate_response(user_response, french_greeting)
            print(feedback)
            if correct:
                correct_answers += 1
    ...
    print("\nBot: You did it! 🎉 Congratulations on mastering the greetings section. Let's head back to the Main Menu...")
